    ...
    def create_documents(self, movie_data):
        ...
        for movie in movie_data["movies"]:
            text_content = (
                ...
                f"Short Summary: {movie['Short Summary']}\n"
                f"IMDB ID: {movie['IMDB ID']}\n"
                f"Runtime: {movie['Runtime']}\n"
                f"YouTube Trailer: {movie['YouTube Trailer']}\n"
                f"Rating: {movie['Rating']}\n"
                f"Director: {movie['Director']}\n"
                f"Writers: {movie['Writers']}\n"
                f"Cast: {movie['Cast']}\n"
                f"Movie Poster: {movie['Movie Poster']}"  # Include Movie Poster attribute
            )
            document = Document(
                text=text_content,
                metadata={'title': movie['Title'], 'year': movie['Year'], 'imdb_id': movie['IMDB ID']},
